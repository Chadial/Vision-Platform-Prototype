...
| 55 | Hardware Audit & Incident Logging Baseline | establish structured auditing for extraordinary camera states and incidents | queued | operational-readiness follow-up that stays behind the current user-directed wx-shell operator lane | `docs/session_workpackages/wp55_hardware_audit_and_incident_logging_baseline.md` |
...
| 60 | wx Recording Progress Status Baseline | surface bounded recording and interval-capture progress in the wx shell through existing status models | current next | next narrow frontend follow-up now that the current ROI/operator interaction lane is good enough to pause | `docs/session_workpackages/wp60_wx_recording_progress_status_baseline.md` |
