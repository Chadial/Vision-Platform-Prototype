...
7. Activate `WP27` next to narrow the remaining hardware-lifecycle and camera-release uncertainty exposed by repeated `camera already in use` observations during serial hardware work.
8. Keep `WP28` directly behind it as the next host-/validation-facing follow-up so ROI width/height/offset failures become clearer and more capability-aware without changing the strict enforcement policy.
