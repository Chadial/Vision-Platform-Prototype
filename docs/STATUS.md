...
- `WP104 Companion Command Polling Service Extraction` is now the default next non-hardware slice; it should move only the bounded `read pending commands -> execute -> write result` loop out of the wx shell without widening runtime semantics
- `WP105 Companion Status Publication Service Extraction` and `WP106 Companion Runtime Tick Coordinator Baseline` are now the queued follow-up sequence behind `WP104`
