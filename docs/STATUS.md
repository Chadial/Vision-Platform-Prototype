...
- recent merged-branch chronology is now preserved in `docs/StatusHistory.md`
...
- landed/completed chronology for `WP80` through `WP99` now lives in `docs/StatusHistory.md`
- `WP100 Companion Session Protocol Baseline` is now landed on `main`; the bounded file-backed local-shell bridge now serializes/deserializes one explicit typed protocol baseline for session metadata, active-session metadata, command envelopes, command-result envelopes, and published status-snapshot envelopes without widening transport or runtime scope
- `WP101 Companion Command Execution Service Extraction` is now landed on `main`; the bounded external companion-command execution path now lives in `vision_platform.services.local_shell_command_execution_service`, while the wx shell applies the returned state updates and keeps the command set unchanged
- `WP102 Companion Status Projection Service Extraction` is now landed on `main`; companion setup, snapshot, recording, and failure reflection projection now lives in `vision_platform.services.local_shell_status_projection_service`, while the wx shell passes explicit state inputs into that seam and keeps the published status payload semantics unchanged
- `WP103 wx Camera Settings Live Path Revalidation` is now completed with fresh May 6, 2026 evidence; the tested hardware path opened the live wx `Camera Settings...` menu on `DEV_1AB22C046D81`, confirmed the dialog without failure reflection, kept preview running, preserved the full effective `Mono10` / gain / ROI status after the profile and partial-configuration fixes, and a follow-up screenshot verified visible camera content after the high-bit wx preview scaling fix
- `WP104 Companion Command Polling Service Extraction` is now landed on `main`; the bounded file-backed `read pending commands -> execute -> write result` loop now lives in `vision_platform.services.local_shell_command_polling_service`, while the wx shell supplies the executor and failure-result shaping without adding queue policy or runtime semantics
- `WP105 Companion Status Publication Service Extraction` is now landed on `main`; the bounded file-backed live-status publication write path now lives in `vision_platform.services.local_shell_status_publication_service`, while the wx shell supplies only the explicit projection input and keeps publication timing and payload semantics unchanged
- `WP106 Companion Runtime Tick Coordinator Baseline` is now landed on `main`; the bounded open-shell tick now composes the extracted polling and publication seams through `vision_platform.services.local_shell_runtime_tick_coordinator` without introducing daemon, lifecycle, or transport semantics
- `WP107 Companion Projection Input Builder Extraction` is now landed on `main`; explicit `LocalShellStatusProjectionInput` assembly now lives in `vision_platform.services.local_shell_projection_input_builder_service`, while the wx shell remains the source of current app-local state and the published payload semantics stay unchanged
- `WP108 Companion Failure Reflection State Extraction` is now landed on `main`; the bounded latest-failure state now lives in `vision_platform.services.local_shell_failure_reflection_state_service`, preserving the single-latest-failure, overwrite-on-newer-failure, clear-on-same-source, and copy behavior exactly
- `WP109 Companion App-Facing Facade Baseline` is now landed on `main`; the wx shell now composes the bounded companion command, result, status-publication, and tick use cases through `vision_platform.services.local_shell_companion_facade` without changing host-visible payloads or runtime semantics
- `WP110 Local Shell Viewport Rendering Adapter Baseline` is now landed on `main`; the shared display-service package now owns the viewport-image rendering helper while the wx shell keeps the bitmap painting and overlay composition responsibilities
- there is now no unconditional `current next` after completed `WP103` and landed `WP110`; any further headless, runtime, host-contract, LabVIEW-facing, renderer-facing, or hardware slice must be justified by a concrete residual, integration need, test failure, or user-observed workflow friction rather than by cleanup momentum
- current non-hardware default sequencing should continue to read through the confirmed `Hybrid Companion` functional workflows rather than through a broad generic subsystem-hardening lens
- for older status chronology that should not crowd this decision layer, use `docs/StatusHistory.md`
- the decision layer at the top of this file should remain the primary reading surface; if the lower historical detail keeps growing, future documentation cleanup should preserve the top-layer summary as the fast authoritative read and push additional density downward rather than expanding the headline section
...
The headless-preparation lane now also has bounded typed-protocol and command-execution seams for the current file-backed companion bridge, so the remaining follow-up can focus on status-projection ownership instead of still mixing protocol shape and command execution inside the wx shell.
...
`WP83` now spans the first narrow post-workflow host-surface consistency seam on `main`: live command results and published status can now be read through the same reflection model for setup, snapshot, recording, and save-directory operations, without widening the transport or redesigning the host contract.
...
- the OpenCV preview/operator path now also keeps its shortcut hints capability-aware, so startup text and the in-window status band advertise snapshot/focus controls only when those paths are actually wired in the active preview entry point, while the wx shell keeps `Camera Settings...` directly reachable as a dialog even when configuration application is currently unavailable and now preloads camera-class defaults plus capability limits before applying a change; the shortcut reference text has also been rewritten to be shell-oriented rather than OpenCV-branded
...
- a shared `PreviewInteractionService` now owns preview interaction-state transitions such as zoom commands, focus/crosshair toggles, ROI-mode changes, point selection, pan updates, copy requests, and snapshot requests, and it keeps rectangle/ellipse ROIs persistently stored while allowing only one shape to be active and visible at a time; crosshair visibility now has priority over ROI entry and cancels in-progress ROI drafting / dragging when enabled
...
2. Treat headless-kernel, LabVIEW-facing, renderer-facing, runtime, broader host-contract work, and further hardware work as later directions, not as an automatic next lane.
3. When a later headless-kernel slice is explicitly justified, revisit the current wx-shell live-sync bridge and lift or replace it with a host-neutral command/session seam instead of freezing the app-local file bridge as the long-term architecture.
