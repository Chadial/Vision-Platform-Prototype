...
7. Treat `WP27` as landed lifecycle hardening for the current real-device initialization seam; keep watching the residual `NotAvailable` log and duplicate enumeration behavior, but do not reopen broad hardware planning from that result alone.
8. Activate `WP28` next so ROI width/height/offset failures become clearer and more capability-aware without changing the strict enforcement policy.
