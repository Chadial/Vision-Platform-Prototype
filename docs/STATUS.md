...
- `WP16` now adds the next `Data And Logging Closure` extension slice: snapshot and bounded recording both write into one folder-local appendable traceability log with stable context header, run/session blocks, and per-image rows
...
2. Use the new shared appendable trace log for snapshot and bounded recording as baseline evidence, then move toward one narrow metadata-aware offline extension instead of reopening frontend breadth.
