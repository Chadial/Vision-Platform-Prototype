...
20. Treat `WP50` as implemented on the current architecture branch: headless display geometry now exists and is test-covered.
21. Make `WP51` the current next slice so preview interaction ownership can move out of OpenCV-specific event handling.
22. Follow `WP51` with `WP52` so overlay/status models are cleaned up before opening `WP53`.
23. Defer `WP54` and `WP55` polish-oriented operational work until the architecture chain above is prepared or a concrete defect forces reprioritization.
24. Treat broader API growth, additional frontends, larger offline tooling, and C# handover as justified post-closure directions rather than current closure obligations.
