...
- Against `docs/WORKPACKAGES.md`: project-level prioritization now runs through the centralized work-package queue instead of many distributed module roadmaps. The earlier command-surface, bounded OpenCV UI, ROI, focus, tracking, first API-preparation, and first postprocess packages can be treated as completed baseline-hardening work, while the next project phase is now intentionally framed as an Extended MVP closure phase rather than breadth expansion. The first `Host Control Closure`, `Experiment Reliability Closure`, and `Data And Logging Closure` slices are now landed through `docs/session_workpackages/wp12_host_control_closure.md`, `docs/session_workpackages/wp13_experiment_reliability_closure.md`, and `docs/session_workpackages/wp14_data_logging_closure.md`. This branch activates the first `Offline And Measurement Closure` slice through `docs/session_workpackages/wp15_offline_measurement_closure.md`, narrowing the immediate offline focus to reuse of saved `BMP` artifacts through the thin postprocess focus-report path. Hardware revalidation remains conditional and should support later reliability closure slices rather than acting as the default stream on its own.
...
1. Use `docs/session_workpackages/wp15_offline_measurement_closure.md` as the current default next implementation package until the repository-level queue is advanced to the next closure slice.
2. Treat the saved-`BMP` offline focus-report slice as baseline evidence for `Offline And Measurement Closure`, then move toward one narrow metadata- or measurement-traceability follow-up instead of reopening frontend breadth.
