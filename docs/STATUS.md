...
- there is now no unconditional non-hardware `current next` after `WP110`; any further headless, runtime, host-contract, LabVIEW-facing, or renderer-facing slice must be justified by a concrete residual, integration need, test failure, or user-observed workflow friction rather than by cleanup momentum
