...
- there is now no unconditional non-hardware `current next` after `WP109`; any further headless, runtime, host-contract, or LabVIEW-facing slice must be justified by a concrete residual, integration need, test failure, or user-observed workflow friction rather than by companion cleanup momentum
