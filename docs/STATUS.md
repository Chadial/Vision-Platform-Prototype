...
- `WP101 Companion Command Execution Service Extraction` is now the default next non-hardware slice, and `WP102 Companion Status Projection Service Extraction` remains the queued follow-up after it
...
The headless-preparation lane now also has a bounded typed protocol baseline for the current file-backed companion bridge, so later command-execution and status-projection extraction work can proceed without continuing to rely on raw free-form session JSON shapes.
