...
- there is no new unconditional non-hardware `current next` until a later headless-preparation seam is explicitly derived from the now-landed `WP100` through `WP102` baseline
