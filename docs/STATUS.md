...
- `WP103 wx Camera Settings Live Path Revalidation` is now completed with fresh May 6, 2026 evidence; the tested hardware path opened the live wx `Camera Settings...` menu on `DEV_1AB22C046D81`, confirmed the dialog without failure reflection, kept preview running, and preserved the full effective `Mono10` / gain / ROI status after the profile and partial-configuration fixes
