...
- `WP107 Companion Projection Input Builder Extraction` is now the default next non-hardware slice; it should move only explicit `LocalShellStatusProjectionInput` assembly out of the wx shell without touching payload semantics
- `WP108 Companion Failure Reflection State Extraction` and `WP109 Companion App-Facing Facade Baseline` are now the queued follow-up sequence behind `WP107`
