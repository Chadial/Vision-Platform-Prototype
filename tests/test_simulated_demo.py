...
class SimulatedDemoTests(unittest.TestCase):
    def test_run_simulated_demo_creates_snapshot_interval_and_recording_outputs(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue((Path(temp_dir) / "demo_interval_000000.raw").exists())
            self.assertTrue((Path(temp_dir) / "demo_interval_000001.raw").exists())
            ...
            self.assertGreaterEqual(result.preview_frame_info.frame_id, 0)
