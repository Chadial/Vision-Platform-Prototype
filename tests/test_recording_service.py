...
class RecordingServiceTests(unittest.TestCase):
    def test_start_recording_writes_raw_sequence_until_frame_limit(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("# run.start", log_lines)
            self.assertIn("# run.end", log_lines)
            self.assertIn("# run.end_state: completed", log_lines)
    ...
    def test_recording_continues_frame_sequence_for_reused_directory_and_same_stem(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(combined_log_lines.count("# run.end_state: completed"), 2)
            self.assertIn("# continues_previous_series: false", combined_log_lines)
            self.assertIn("# recording_start_frame_index: 0", combined_log_lines)
            self.assertIn("# continues_previous_series: true", combined_log_lines)
            self.assertIn("# recording_start_frame_index: 2", combined_log_lines)
            data_rows = list(csv.reader(line for line in combined_log_lines if not line.startswith("# ")))
            self.assertEqual(data_rows[0], ["image_name", "frame_id", "camera_timestamp", "system_timestamp_utc"])
            self.assertEqual(len(data_rows), 5)
            self.assertEqual(data_rows[1][0], "series_000000.raw")
            self.assertEqual(data_rows[4][0], "series_000003.raw")
