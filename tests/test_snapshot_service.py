...
class SnapshotServiceTests(unittest.TestCase):
    def test_save_snapshot_writes_bmp_for_supported_visible_format(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            recording_log_lines = (Path(temp_dir) / "recording_log.csv").read_text(encoding="utf-8").splitlines()
            recording_log_rows = list(csv.reader(recording_log_lines))
            self.assertEqual(recording_log_rows[0], ["image_name", "frame_id", "camera_timestamp", "system_timestamp_utc"])
            self.assertEqual(recording_log_rows[1][0], "capture_001_000000.bmp")
    ...
    def test_save_snapshot_writes_trace_log_with_configuration_fields(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            recording_log_rows = list(csv.reader(recording_log_lines))
            self.assertEqual(recording_log_rows[0], ["image_name", "frame_id", "camera_timestamp", "system_timestamp_utc"])
            self.assertEqual(recording_log_rows[1][0], "capture_001_000000.png")
            self.assertEqual(recording_log_rows[1][1], "7")
            self.assertEqual(recording_log_rows[1][2], "123456")
            trace_rows = list(csv.reader(line for line in trace_lines if line and not line.startswith("# ")))
