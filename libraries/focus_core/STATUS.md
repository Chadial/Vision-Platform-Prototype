...
- partial: if artifact metadata uses `focus_value_mean` and `focus_value_stddev`, they should be interpreted against an explicit moving-score window such as `focus_score_frame_interval`, with the intended narrow baseline set to default `7` frames and bounded to `3..17`
