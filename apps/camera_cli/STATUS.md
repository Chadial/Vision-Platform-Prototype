...
- next step: keep the exposed `run_id` narrow and linkage-oriented, treat the March 30, 2026 hardware rerun as fresh evidence for the current bounded CLI host surface, investigate the residual `NotAvailable` startup log only if it proves user-visible, and only extend beyond `Capture`, `Camera`, and `Storage` when that can be done without inventing CLI-only business logic
