...
- implemented: bounded wxPython local working shell for the simulated preview path under `vision_platform.apps.local_shell`
- working now: the shell starts through the project `.venv`, opens a real wx window, polls the shared preview stream, and renders the latest simulated frame in a bounded image area
...
- working now: fit/zoom, crosshair toggle, and rectangle/ellipse ROI entry reuse the extracted display geometry, interaction, and preview-status layers instead of introducing UI-private business logic
...
- partial: the shell is currently simulator-first; no bounded hardware validation exists yet for the wx path
